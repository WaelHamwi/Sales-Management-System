...
@section('content')
    ...
    <div class="page-header">
      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li  class="breadcrumb-item active" aria-current="page"><a > {{__('s.create_product')}}</a></li>
        </ol>
      </nav>
    </div>
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <!-- Form  starts -->
                <form action="{{ route('products.store') }}" id="formDropzone" class="forms-sample" method="POST" enctype="multipart/form-data" style="border:none; background:#FFFFFF; padding:0px;">
      @csrf
      <div class="form-group">
          <label for="exampleInputName1">{{ __('s.product_name') }} <span>*</span></label>
          <input type="text" value="" autofocus name="product_name" class="form-control" id="exampleInputName1" placeholder="{{ __('s.product_name_placeholder') }}">
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
          ...
          <input type="text" value="" autofocus name="product_price" class="form-control" id="productPrice" placeholder="{{ __('s.product_price_placeholder') }}">
      </div>
      <div class="form-group">
          <label for="exampleInputName1">{{__('s.SKU')}} </label>
          <input type="text" value="" autofocus name="product_sku" class="form-control" id="productSKU" placeholder="{{ __('s.product_sku_placeholder') }}">
      </div>
      <div class="form-group">
          <label for="exampleInputName1">{{__('s.Barcode')}} </label>
          <input type="text" value="" autofocus name="product_barcode" class="form-control" id="productBarcode" placeholder="{{ __('s.product_barcode_placeholder') }}">
      </div>
      <!--dropzone form start-->
      ...
      <!--dropzone form end-->
      <button id="formSubmitProduct" type="submit" class="btn btn-gradient-primary me-2">{{ __('s.submit') }}</button>
