...
@section('content')
    ...
    <div class="page-header">
      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li  class="breadcrumb-item active" aria-current="page"><a > {{__('s.edit_order_item')}}</a></li>
        </ol>
      </nav>
    </div>
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <!-- Form  starts -->
              <form action="{{ route('orderItems.update', ['id' => $order_item->id]) }}" class="forms-sample" method="POST"   enctype="multipart/form-data" style="border:none; background:#FFFFFF; padding:0px;">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="exampleInputName1">{{ __('s.order_item_subtotal') }} <span>*</span></label>
                        <input type="text" value="{{$order_item->subtotal}}" autofocus name="order_item_subtotal" class="form-control" id="exampleInputName1" placeholder="{{ __('s.order_item_subtotal_placeholder') }}">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" value="{{$order_item->product_price}}" autofocus name="product_price" class="form-control" id="product_price" placeholder="{{ __('s.product_price_placeholder') }}">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputName1">{{ __('s.product_name') }} <span>*</span></label>
                        <input type="text" value="{{$order_item->product_name}}" autofocus name="product_name" class="form-control" id="product_name" placeholder="{{ __('s.product_name_placeholder') }}">
                    </div>
